/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package jmp.infral;

/**
 *
 * @author jmp
 */
public class Constants {

    public static final int RECORDER_BUFFER_SIZE = 1024;

    //DataModel
    //public static final int DEFAULT_SONG_DATA_SIZE = 100000;
   
    /*
     * Default Generated Frames Per Step. It corresponds to the minimum number
     * of frames that can be computed for display.
     */
    public static final int DGFPS = 2;
    
    /*
     * Default Distance Between Two Points. public static final int DDBTP = 2;
     *
     */
    public static final int DDBTP = 2;
    
    public static final String csvPath = "D:\\Documents\\ENSISA\\3A_IR\\S1\\Projet_Perronne\\chart\\csv\\CSV_test.csv";
    public static final String csvPath2 = "D:\\Documents\\ENSISA\\3A_IR\\S1\\Projet_Perronne\\chart\\csv\\test2.csv";
    public static final String path = "D:\\\\Documents\\\\ENSISA\\\\3A_IR\\\\S1\\\\Projet_Perronne\\\\chart\\\\songs\\\\ALB.wav";
}
